...
public:
    std::string operator()(const TcpServerConf& conf) {
        ...
        // node["process_worker"] = conf.process_/worker;
...
public:
     ...
     */
    TcpServer(DW::IOManager* worker = DW::IOManager::GetThis()
              ,DW::IOManager* io_woker = DW::IOManager::GetThis()
     ...
     * @return 返回是否绑定成功
     */
    virtual bool bind(DW::Address::ptr addr);
