...
HttpConnection::~HttpConnection() {
    DW_LOG_DEBUG(g_logger, __FILE__, __LINE__, TOSTRING("HttpConnection::~HttpConnection"));
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            DW_LOG_DEBUG(g_logger, __FILE__, __LINE__, TOSTRING("content_len=" , client_parser.content_len));
    ...
    //     DW_LOG_DEBUG(g_logger, __FILE__, __LINE__, TOSTRING("content_encoding: " , content_encoding
    //         , " size=" , body.size()));
...
HttpConnectionPool::ptr HttpConnectionPool::Create(const std::string& uri
    ...
    if(!turi) {
        DW_LOG_ERROR(g_logger, __FILE__, __LINE__, TOSTRING("invalid uri=" , uri));
    }
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = DW::GetCurrentMS();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            DW_LOG_ERROR(g_logger, __FILE__, __LINE__, TOSTRING("get addr fail: " , m_host));
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            DW_LOG_ERROR(g_logger, __FILE__, __LINE__, TOSTRING("create sock fail: " , *addr));
            return nullptr;
        }
        if(!sock->connect(addr)) {
            DW_LOG_ERROR(g_logger, __FILE__, __LINE__, TOSTRING("sock connect fail: " , *addr));
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) <= DW::GetCurrentMS())
