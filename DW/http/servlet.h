...
public:
     ...
     * @return 是否处理成功
     */
    virtual int32_t handle(DW::http::HttpRequest::ptr request
                   , DW::http::HttpResponse::ptr response
                   , DW::http::HttpSession::ptr session) = 0;
...
public:
    ...
    /// 函数回调类型定义
    using callback = std::function<int32_t (DW::http::HttpRequest::ptr request
                   , DW::http::HttpResponse::ptr response
    ...
    virtual int32_t handle(DW::http::HttpRequest::ptr request
                   , DW::http::HttpResponse::ptr response
                   , DW::http::HttpSession::ptr session) override;
private:
    /// 回调函数
    callback m_cb;
};
