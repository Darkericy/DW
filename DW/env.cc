...
bool Env::init(int argc, char** argv) {
    ...
    DW_ASSERT(readlink(link, path, sizeof(path)) != -1);
    // /path/xxx/exe
    m_exe = path;
    ...
    for(int i = 1; i < argc; ++i) {
        if(argv[i][0] == '-') {
            if(strlen(argv[i]) > 1) {
                ...
                DW_LOG_ERROR(g_logger, __FILE__, __LINE__, TOSTRING("invalid arg idx=" , i
                    , " val=" , argv[i]));
                return false;
            }
        } else {
...
std::string Env::getAbsoluteWorkPath(const std::string& path) const {
    ...
    static DW::ConfigVar<std::string>::ptr g_server_work_path =
        DW::Config::Lookup<std::string>("server.work_path");
